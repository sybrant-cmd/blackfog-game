...
export function initFacilitator() {
  ...
  function buildVCISOAARComment(inj, tally, majority) {
    if (!inj || !inj.decision || !inj.decision.question) {
      return "No formal decision was required for this inject.";
    }
    const total = (tally.A || 0) + (tally.B || 0) + (tally.C || 0);
    if (!total) {
      return "No responses were recorded for this decision point. In a real incident this would look like a lack of engagement or clarity.";
    }

    let majorityLine = "";
    if (!majority) {
      majorityLine = "The group was split across options with no clear majority. This usually means the trade-offs were not clearly framed or agreed.";
    } else {
      const optText = inj.decision.options && inj.decision.options[majority];
      const shortLabel = optText && optText.length > 90 ? optText.slice(0, 87) + "…" : optText;
      majorityLine = "Most participants chose option " + majority + (shortLabel ? " – \"" + shortLabel + "\"" : "") + ".";
    }

    let phaseHint = "";
    const t = (inj.type || "").toLowerCase();
    if (t.includes("intel")) {
      phaseHint = " From a vCISO lens this is an early intelligence moment – it is better to be slightly over-cautious here than to downplay weak but unusual signals.";
    } else if (t.includes("detection")) {
      phaseHint = " Detection choices shape how quickly the story becomes visible. Under-investing in detection here usually shows up later as surprise and regret.";
    } else if (t.includes("containment")) {
      phaseHint = " Containment decisions always create pain somewhere in the business. What matters is whether that pain is deliberate and well-communicated, not accidental.";
    } else if (t.includes("business")) {
      phaseHint = " Business-facing calls here will strongly affect trust. Clear, plain language usually beats either reassurance theatre or technical jargon.";
    } else if (t.includes("recovery")) {
      phaseHint = " Recovery is where organisations are tempted to declare victory and move on. The real opportunity is to lock in structural improvements while memories are still fresh.";
    } else {
      phaseHint = " This is a useful point to pause and ask whether the organisation is really aligned on the trade-offs you just made.";
    }

    return majorityLine + " " + phaseHint;
  }
  ...
  if (btnBuildAAR) {
    btnBuildAAR.onclick = async () => {
      ...
      thead.innerHTML = "<tr><th>#</th><th>Time</th><th>Type</th><th>Inject</th><th>A</th><th>B</th><th>C</th><th>Majority</th></tr>";
      ...
      staticInjects.forEach(inj => {
        ...
        let majority = null;
        if (total > 0) {
          const max = Math.max(tally.A || 0, tally.B || 0, tally.C || 0);
          const top = [];
          if (tally.A === max) top.push("A");
          if (tally.B === max) top.push("B");
          if (tally.C === max) top.push("C");
          if (top.length === 1) majority = top[0];
        }
        ...
        const item = {
          ...
          type: inj.type || "",
          text: inj.text || "",
          question: inj.decision && inj.decision.question ? inj.decision.question : "",
          options: inj.decision && inj.decision.options ? inj.decision.options : {},
          votes: tally,
          totalVotes: total,
          majority: majority,
          vciso: buildVCISOAARComment(inj, tally, majority)
        };
        aarItems.push(item);
      });
      ...
      await set(ref(db, `sessions/${session}/aar`), aarPayload);

      log("AAR snapshot built from static inject votes and published for players.");
    };
  }
  ...
  if (btnEndExercise) {
    btnEndExercise.onclick = async () => {
      ...
      // Clear the live inject but retain votes for AAR purposes
      await set(ref(db, `sessions/${session}/latestInject`), null);
      // Reset scenario so new players see standby instead of an old scenario
      await set(ref(db, `sessions/${session}/scenario`), null);
      await update(ref(db, `sessions/${session}/control`), {
        reset: Date.now(),
...
export function initPlayer() {
  ...
  onValue(ref(db, `sessions/${session}/scenario`), async snap => {
    const v = snap.val();

    // Strict standby logic: ignore any stale or unpublished scenario.
    let resetTs = 0;
    const ctlSnap = await get(ref(db, `sessions/${session}/control`));
    if (ctlSnap.exists()) {
      const ctl = ctlSnap.val() || {};
      resetTs = ctl.resetTimestamp || 0;
    }

    if (!v || !v.published || !v.publishedAt || v.publishedAt <= resetTs) {
      resetView();
      return;
    }
  ...
  function renderPlayerAAR(aar) {
    const card = $("aarCard");
    const host = $("aarSummary");
    if (!card || !host) return;
    if (!aar || !aar.items || !aar.items.length) {
      host.textContent = "AAR is not yet available. Please wait for the facilitator to build it.";
      return;
    }
    card.style.display = "block";
    let html = "<table><thead><tr><th>#</th><th>Phase</th><th>Inject</th><th>A/B/C votes</th><th>Majority</th><th>vCISO view</th></tr></thead><tbody>";
    aar.items.forEach(item => {
      const votes = item.votes || {};
      const maj = item.majority || "—";
      const textShort = item.text && item.text.length > 80 ? item.text.slice(0, 77) + "…" : (item.text || "");
      const vciso = item.vciso || "";
      html += "<tr>" +
        "<td>" + (item.seq || "") + "</td>" +
        "<td>" + (item.type || "") + "</td>" +
        "<td>" + textShort.replace(/</g, "&lt;").replace(/>/g, "&gt;") + "</td>" +
        "<td>" +
          "A:" + (votes.A || 0) + " " +
          "B:" + (votes.B || 0) + " " +
          "C:" + (votes.C || 0) +
        "</td>" +
        "<td>" + maj + "</td>" +
        "<td>" + vciso.replace(/</g, "&lt;").replace(/>/g, "&gt;") + "</td>" +
      "</tr>";
    });
    html += "</tbody></table>";
    host.innerHTML = html;
  ...
  onValue(ref(db, `sessions/${session}/aar`), snap => {
    const v = snap.val();
    if (!v) return;
    renderPlayerAAR(v);
  });
